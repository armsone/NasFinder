...
private final class WebHardHTTPConnection: @unchecked Sendable {
    ...
    #path{font-size:13px;color:#666;overflow-wrap:anywhere}#status{min-height:20px;font-size:13px;color:#555}.danger{color:#c62828}
    ...
    @media(prefers-color-scheme:dark){body{background:#111115;color:#f5f5f7}.files,.row,.transfer,.files.small .row,.files.poster .row{background:#1c1c20}.row{border-color:#303036}.button.secondary,button.secondary{background:#303038;color:#eee}.meta,#path,#status{color:#aaa}.preview{background:#29292f}}
    </style></head><body><header><div class="badge">H</div><div><h1>NasFinder PhoneHard</h1><div id="path">/</div></div></header>
    <div class="bar"><button id="up" class="secondary">위로</button><label class="button">올리기<input id="files" type="file" multiple></label><button id="receive" disabled>받기</button><button id="mkdir" class="secondary">새 폴더</button><span class="spacer"></span><button data-view="list" class="secondary">자세히</button><button data-view="small" class="secondary">썸네일</button><button data-view="poster" class="secondary">포스터</button></div>
    <div id="status"></div><div id="transfers"></div><main id="list" class="files small"></main>
    <script>
    ...
    async function upload(input){const files=[...input.files];if(!files.length)return;status.className='';let done=0;for(const file of files){try{await sendFile(file,join(current,file.name));done++}catch(e){status.className='danger';status.textContent=`${file.name}: ${e.message}`;input.value='';return}}input.value='';status.textContent=`${done}개 올림`;await load(current)}
    document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>setView(b.dataset.view));setView(view);document.querySelector('#files').onchange=e=>upload(e.target);receive.onclick=async()=>{const items=[...selected.values()];receive.disabled=true;status.className='';status.textContent=`${items.length}개 받는 중…`;try{for(const item of items)await download(item);status.textContent=`${items.length}개 받기 시작함`}catch(e){status.className='danger';status.textContent=e.message}finally{updateSelection()}};document.querySelector('#up').onclick=()=>{if(current==='/')return;load(current.split('/').slice(0,-1).join('/')||'/')};document.querySelector('#mkdir').onclick=async()=>{const name=prompt('새 폴더 이름');if(!name||name.includes('/'))return;try{await json(api('/api/folder',join(current,name)),{method:'POST'});await load(current)}catch(e){status.className='danger';status.textContent=e.message}};load('/');
    </script></body></html>
    """#
}
