...
private final class WebHardHTTPConnection: @unchecked Sendable {
    ...
    async function upload(input){const files=[...input.files];if(!files.length)return;status.className='';let done=0;for(const file of files){try{await sendFile(file,join(current,file.name));done++}catch(e){status.className='danger';status.textContent=`${file.name}: ${e.message}`;input.value='';return}}input.value='';await load(current);status.textContent=`${done}개 올림`}
