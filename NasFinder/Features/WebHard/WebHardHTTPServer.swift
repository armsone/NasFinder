...
private final class WebHardHTTPConnection: @unchecked Sendable {
    ...
    :root{color-scheme:light dark;font-family:-apple-system,BlinkMacSystemFont,sans-serif;--enamel:#fbfaf6;--bg:#ece9e2;--charcoal:#303336;--muted:#62666a;--chrome:#92979b;--orange:#ff7900;--red:#e41e25}
    body{max-width:980px;margin:auto;padding:22px 16px;background:linear-gradient(145deg,#f6f3ed,var(--bg));color:#151719}
    header{display:flex;align-items:center;gap:12px;margin-bottom:16px;padding:12px;border:1px solid #ffffffcc;border-radius:18px;background:linear-gradient(145deg,#fff,var(--enamel));box-shadow:0 10px 26px #2f33351c,inset 0 1px #fff}h1{font-size:22px;margin:0}
    .phonehard-mark{display:grid;gap:1px;width:46px;height:46px;padding:5px;border:2px solid var(--chrome);border-radius:12px;background:linear-gradient(145deg,#414548,#111315);box-shadow:0 5px 12px #0004,inset 0 1px #fff8;font-weight:900;line-height:1}.phonehard-mark .phone{align-self:end;color:#fff;font-size:10px}.phonehard-mark .hard{display:grid;place-items:center;height:17px;border-radius:4px;background:linear-gradient(#ff941d,#f34d00);color:#161819;font-size:12px;box-shadow:inset 0 1px #fff7}
    .bar{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:12px 0}.bar .spacer{flex:1}
    button,.button{border:1px solid #ffffff99;border-radius:10px;padding:10px 13px;background:linear-gradient(#45494c,#25282a);color:white;font:inherit;text-decoration:none;cursor:pointer;box-shadow:0 3px 8px #0002,inset 0 1px #fff4}
    button.secondary,.button.secondary{border-color:#c7c9c8;background:linear-gradient(#fff,#dddcd7);color:#282b2d}button.selected{outline:2px solid var(--orange)}button:disabled{opacity:.45;cursor:default}input[type=file]{display:none}
    ...
    dialog{width:min(520px,calc(100% - 32px));border:0;border-radius:18px;padding:0;background:white;color:inherit;box-shadow:0 18px 60px #0005}dialog::backdrop{background:#0007}.upload-panel{padding:20px}.upload-panel h2{margin:0 0 6px;font-size:20px}.upload-panel p{margin:0 0 16px;color:#777;font-size:13px}.upload-choices,.upload-actions{display:flex;gap:8px;flex-wrap:wrap}.upload-actions{justify-content:flex-end;margin-top:16px}#uploadQueue{max-height:240px;overflow:auto;margin:14px 0 0;padding:0;list-style:none}#uploadQueue li{display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid #ededf2;font-size:13px}#uploadQueue li span{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}#uploadQueue li button{padding:5px 8px}
    ...
    .files{background:var(--enamel);border:1px solid #fff;border-radius:16px;overflow:hidden;box-shadow:0 10px 26px #2f333518}.row{position:relative;display:flex;align-items:center;gap:10px;padding:12px 14px;border-bottom:1px solid #d8d7d2;cursor:pointer}.row:last-child{border-bottom:0}.select{flex:0 0 auto;width:19px;height:19px;cursor:pointer;accent-color:var(--orange)}
    ...
    @media(prefers-color-scheme:dark){body{background:linear-gradient(145deg,#202326,#111315);color:#f5f5f7}header,.files,.row,.transfer,.files.small .row,.files.poster .row,dialog{background:#26292b;border-color:#4a4e51}.row,#uploadQueue li{border-color:#3b3f42}.button.secondary,button.secondary{background:linear-gradient(#4a4e51,#303335);color:#eee}.meta,#path,#status,#uploadHint,.upload-panel p{color:#b5b8ba}.preview{background:#34383b}}
    </style></head><body><header><div class="phonehard-mark" aria-hidden="true"><span class="phone">Phone</span><span class="hard">Hard</span></div><div><h1>NasFinder PhoneHard</h1><div id="path">/</div></div></header>
    <div class="bar"><button id="up" class="secondary">위로</button><button id="uploadOpen">올리기</button><button id="receive" disabled>받기</button><button id="mkdir" class="secondary">새 폴더</button><span class="spacer"></span><button data-view="list" class="secondary">자세히</button><button data-view="small" class="secondary">썸네일</button><button data-view="poster" class="secondary">포스터</button></div>
    ...
    <dialog id="uploadDialog"><div class="upload-panel"><h2>파일과 폴더 올리기</h2><p>파일과 폴더를 같은 목록에 추가한 뒤 한 번에 올립니다.</p><div class="upload-choices"><label class="button">파일 추가<input id="files" type="file" multiple></label><label class="button secondary">폴더 추가<input id="folder" type="file" webkitdirectory multiple></label></div><ul id="uploadQueue"></ul><div class="upload-actions"><button id="uploadCancel" class="secondary">취소</button><button id="uploadSubmit" disabled>전부 올리기</button></div></div></dialog>
    <script>
    let password=sessionStorage.getItem('webHardPassword')||'',current='/',view=localStorage.getItem('phoneHardView')||'small';
    const list=document.querySelector('#list'),status=document.querySelector('#status'),pathLabel=document.querySelector('#path'),transfers=document.querySelector('#transfers'),receive=document.querySelector('#receive'),uploadDialog=document.querySelector('#uploadDialog'),uploadQueue=document.querySelector('#uploadQueue'),uploadSubmit=document.querySelector('#uploadSubmit'),selected=new Map();let queuedUploads=[];
    ...
    function renderUploadQueue(){uploadQueue.replaceChildren();queuedUploads.forEach((value,index)=>{const row=document.createElement('li'),name=document.createElement('span'),remove=document.createElement('button');name.textContent=value.path;remove.className='secondary';remove.textContent='빼기';remove.onclick=()=>{queuedUploads.splice(index,1);renderUploadQueue()};row.append(name,remove);uploadQueue.append(row)});uploadSubmit.disabled=queuedUploads.length===0;uploadSubmit.textContent=queuedUploads.length?`전부 올리기 (${queuedUploads.length})`:'전부 올리기'}
    function addToUploadQueue(input){queuedUploads.push(...[...input.files].map(file=>({file,path:file.webkitRelativePath||file.name})));input.value='';renderUploadQueue()}
    function closeUploadDialog(){uploadDialog.close?.();uploadDialog.removeAttribute('open')}
    async function upload(files){if(!files.length)return;status.className='';let done=0;for(const value of files){try{await sendFile(value.file,join(current,value.path));done++}catch(e){status.className='danger';status.textContent=`${value.path}: ${e.message}`;return}}await load(current);status.textContent=`${done}개 올림`}
    document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>setView(b.dataset.view));setView(view);document.querySelector('#uploadOpen').onclick=()=>{queuedUploads=[];renderUploadQueue();uploadDialog.showModal?.();if(!uploadDialog.open)uploadDialog.setAttribute('open','')};document.querySelector('#files').onchange=e=>addToUploadQueue(e.target);document.querySelector('#folder').onchange=e=>addToUploadQueue(e.target);document.querySelector('#uploadCancel').onclick=()=>{queuedUploads=[];closeUploadDialog()};uploadSubmit.onclick=async()=>{const files=queuedUploads;queuedUploads=[];closeUploadDialog();await upload(files)};let dragDepth=0;document.body.ondragenter=e=>{e.preventDefault();dragDepth++;document.body.classList.add('dragging')};document.body.ondragover=e=>{e.preventDefault();if(e.dataTransfer)e.dataTransfer.dropEffect='copy'};document.body.ondragleave=e=>{e.preventDefault();if(--dragDepth<=0){dragDepth=0;document.body.classList.remove('dragging')}};document.body.ondrop=async e=>{e.preventDefault();dragDepth=0;document.body.classList.remove('dragging');try{await upload(await droppedFiles(e.dataTransfer))}catch(error){status.className='danger';status.textContent=error.message}};receive.onclick=async()=>{const items=[...selected.values()];receive.disabled=true;status.className='';status.textContent=`${items.length}개 받는 중…`;try{for(const item of items)await download(item);status.textContent=`${items.length}개 받기 시작함`}catch(e){status.className='danger';status.textContent=e.message}finally{updateSelection()}};document.querySelector('#up').onclick=()=>{if(current==='/')return;load(current.split('/').slice(0,-1).join('/')||'/')};document.querySelector('#mkdir').onclick=async()=>{const name=prompt('새 폴더 이름');if(!name||name.includes('/'))return;try{await json(api('/api/folder',join(current,name)),{method:'POST'});await load(current)}catch(e){status.className='danger';status.textContent=e.message}};load('/');
    </script></body></html>
    """#
}
